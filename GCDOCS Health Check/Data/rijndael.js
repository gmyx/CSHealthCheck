...
**/
        ...
        decryptString: function (cyphertextBase64, password) {
            if (/^[0-9a-f]+$/i.test(cyphertextBase64) &&
                cyphertextBase64.length % 2 == 0) { // use old method
        ...
        encryptString_old: function(plaintext, password) {
            ...
            if (key.length < 64) {  //increase key length to 64
                while (key.length < 64) {
                    key+="0000";
                }
            } else {
            ...
            if (plaindata.length > 64) { // separate data to blocks and encode
